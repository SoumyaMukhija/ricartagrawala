...
public class Server {
    ...
    List<ServerConnection> ServerConnectionList = new LinkedList<>();
    ServerSocket server;
    String Id;
    HashMap<String,ServerConnection> ServerConnectionHashMap = new HashMap<>();
    ...
    public  synchronized void writeToFile( String fileName, Message message) throws IOException {
        ...
        writer.close();
        ServerConnectionHashMap.get(message.getClientId()).sendWriteAcknowledge(fileName);
    }
    ...
    public synchronized void fileHostedString(String requestingClientId){
        ...
        ServerConnectionHashMap.get(requestingClientId).sendHostedFiles(this.allFiles);
    }
    ...
    public synchronized void readLastFile(String fileName, String requestingClientId) {
        ...
        if(!lastLine.isEmpty()) {
            ...
            ServerConnectionHashMap.get(requestingClientId).sendLastMessageOnFile(fileName,returnMessage);
            ...
            public void run(){
                while(true){
                    try{
                        ...
                        ServerConnectionHashMap.put(ServerConnection.getRemote_id(),ServerConnection);
                    }
